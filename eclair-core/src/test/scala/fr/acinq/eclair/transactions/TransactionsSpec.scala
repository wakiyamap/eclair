...
class TransactionsSpec extends AnyFunSuite with Logging {
  implicit def bytearray2bytevectoracinq(input: Array[Byte]): fr.acinq.bitcoin.ByteVector = new fr.acinq.bitcoin.ByteVector(input)
  ...
  test("check pre-computed transaction weights") {
    ...
    }
    {
      ...
      val htlc = UpdateAddHtlc(ByteVector32.Zeroes, 0, (20000 * 1000) msat, paymentPreimage.sha256(), CltvExpiryDelta(144).toCltvExpiry(blockHeight), TestConstants.emptyOnionPacket)
    ...
    }
    {
      ...
      val paymentPreimage = randomBytes32
      val htlc = UpdateAddHtlc(ByteVector32.Zeroes, 0, (20000 * 1000) msat, paymentPreimage.sha256(), toLocalDelay.toCltvExpiry(blockHeight), TestConstants.emptyOnionPacket)
  ...
  test("generate valid commitment and htlc transactions (default commitment format)") {
    ...
    val htlc1 = UpdateAddHtlc(ByteVector32.Zeroes, 0, MilliBtc(100).toMilliSatoshi, paymentPreimage1.sha256(), CltvExpiry(300), TestConstants.emptyOnionPacket)
    val paymentPreimage2 = randomBytes32
    val htlc2 = UpdateAddHtlc(ByteVector32.Zeroes, 1, MilliBtc(200).toMilliSatoshi, paymentPreimage2.sha256(), CltvExpiry(310), TestConstants.emptyOnionPacket)
    ...
    val htlc3 = UpdateAddHtlc(ByteVector32.Zeroes, 2, (localDustLimit plus weight2fee(feeratePerKw, DefaultCommitmentFormat.htlcTimeoutWeight)).toMilliSatoshi, paymentPreimage3.sha256(), CltvExpiry(295), TestConstants.emptyOnionPacket)
    val paymentPreimage4 = randomBytes32
    val htlc4 = UpdateAddHtlc(ByteVector32.Zeroes, 3, (localDustLimit plus weight2fee(feeratePerKw, DefaultCommitmentFormat.htlcSuccessWeight)).toMilliSatoshi, paymentPreimage4.sha256(), CltvExpiry(300), TestConstants.emptyOnionPacket)
    // htlc5 and htlc6 are dust IN/OUT htlcs
    val htlc5 = UpdateAddHtlc(ByteVector32.Zeroes, 4, (localDustLimit times 0.9).toMilliSatoshi, randomBytes32.sha256(), CltvExpiry(295), TestConstants.emptyOnionPacket)
    val htlc6 = UpdateAddHtlc(ByteVector32.Zeroes, 5, (localDustLimit times 0.9).toMilliSatoshi, randomBytes32.sha256(), CltvExpiry(305), TestConstants.emptyOnionPacket)
  ...
  test("generate valid commitment and htlc transactions (anchor outputs)") {
    ...
    val htlc1 = UpdateAddHtlc(ByteVector32.Zeroes, 0, MilliBtc(100).toMilliSatoshi, paymentPreimage1.sha256(), CltvExpiry(300), TestConstants.emptyOnionPacket)
    val paymentPreimage2 = randomBytes32
    val htlc2a = UpdateAddHtlc(ByteVector32.Zeroes, 1, MilliBtc(50).toMilliSatoshi, paymentPreimage2.sha256(), CltvExpiry(310), TestConstants.emptyOnionPacket)
    val htlc2b = UpdateAddHtlc(ByteVector32.Zeroes, 2, MilliBtc(150).toMilliSatoshi, paymentPreimage2.sha256(), CltvExpiry(310), TestConstants.emptyOnionPacket)
    // htlc3 and htlc4 are dust IN/OUT htlcs, with an amount large enough to be included in the commit tx, but too small to be claimed at 2nd stage
    val paymentPreimage3 = randomBytes32
    val htlc3 = UpdateAddHtlc(ByteVector32.Zeroes, 3, (localDustLimit plus weight2fee(feeratePerKw, AnchorOutputsCommitmentFormat.htlcTimeoutWeight)).toMilliSatoshi, paymentPreimage3.sha256(), CltvExpiry(295), TestConstants.emptyOnionPacket)
    val paymentPreimage4 = randomBytes32
    val htlc4 = UpdateAddHtlc(ByteVector32.Zeroes, 4, (localDustLimit plus weight2fee(feeratePerKw, AnchorOutputsCommitmentFormat.htlcSuccessWeight)).toMilliSatoshi, paymentPreimage4.sha256(), CltvExpiry(300), TestConstants.emptyOnionPacket)
    // htlc5 and htlc6 are dust IN/OUT htlcs
    val htlc5 = UpdateAddHtlc(ByteVector32.Zeroes, 5, (localDustLimit times 0.9).toMilliSatoshi, randomBytes32.sha256(), CltvExpiry(295), TestConstants.emptyOnionPacket)
    val htlc6 = UpdateAddHtlc(ByteVector32.Zeroes, 6, (localDustLimit times 0.9).toMilliSatoshi, randomBytes32.sha256(), CltvExpiry(305), TestConstants.emptyOnionPacket)
    val spec = CommitmentSpec(
      htlcs = Set(
        OutgoingHtlc(htlc1),
        IncomingHtlc(htlc2a),
  ...
  test("sort the htlc outputs using BIP69 and cltv expiry") {
    ...
    val htlc1 = UpdateAddHtlc(randomBytes32, 1, MilliBtc(100).toMilliSatoshi, paymentPreimage1.sha256(), CltvExpiry(300), TestConstants.emptyOnionPacket)
    val htlc2 = UpdateAddHtlc(randomBytes32, 2, MilliBtc(200).toMilliSatoshi, paymentPreimage2.sha256(), CltvExpiry(300), TestConstants.emptyOnionPacket)
    val htlc3 = UpdateAddHtlc(randomBytes32, 3, MilliBtc(200).toMilliSatoshi, paymentPreimage3.sha256(), CltvExpiry(300), TestConstants.emptyOnionPacket)
    val htlc4 = UpdateAddHtlc(randomBytes32, 4, MilliBtc(200).toMilliSatoshi, paymentPreimage3.sha256(), CltvExpiry(300), TestConstants.emptyOnionPacket)
    val htlc5 = UpdateAddHtlc(randomBytes32, 5, MilliBtc(200).toMilliSatoshi, paymentPreimage3.sha256(), CltvExpiry(301), TestConstants.emptyOnionPacket)
  ...
  test("BOLT 3 fee tests") {
    ...
    val tests = testRegex.findAllIn(bolt3).map(s => {
      ...
      TestVector(name, CommitmentSpec(htlcs, feerate_per_kw.toLong, MilliSatoshi(to_local_msat.toLong), MilliSatoshi(to_remote_msat.toLong)), new Satoshi(fee.toLong))
    }).toSeq
