...
object PaymentRequest {
  ...
  object TwitterEncoding {

    val twitterEmojis = Seq(
      "1f004",
      "1f0cf",
      "1f170",
      "1f171",
    ...
    def write(pr: PaymentRequest): String = {
      ...
      val hramount = Amount.encode(pr.amount)
      val hrp = s"${pr.prefix}$hramount"
      var data = Codecs.bolt11DataCodec.encode(Bolt11Data(pr.timestamp, pr.tags, pr.signature)).require
      var chars = Seq.empty[String]
      do {
        val current = "&#x" ++ TwitterEncoding.twitterEmojis(data.take(11).toInt(signed = false)).replace("-", "&#x")
        chars = chars :+ current
        data = data.drop(11)
      } while (data.nonEmpty)
      chars.mkString("")
