...
class ExtendedBitcoinClientSpec extends TestKitBaseClass with BitcoindService with AnyFunSuiteLike with BeforeAndAfterAll with Logging {
  ...
  test("detect if tx has been doublespent") {
    ...
    val inputs = tx1.txIn.asScala.map(txIn => Map("txid" -> txIn.outPoint.txid.toString, "vout" -> txIn.outPoint.index)).toArray
    bitcoinClient.invoke("createrawtransaction", inputs, Map(address -> tx1.txOut.asScala.map(_.amount).sum.toLong * 1.0 / 1e8)).pipeTo(sender.ref)
  ...
  test("find spending transaction of a given output") {
    ...
    generateBlocks(bitcoincli, 1)
    client.lookForSpendingTx(None, tx1.txIn.get(0).outPoint.txid, tx1.txIn.get(0).outPoint.index.toInt).pipeTo(sender.ref)
