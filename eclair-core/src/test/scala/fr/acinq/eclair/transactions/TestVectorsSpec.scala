...
import fr.acinq.bitcoin.{ByteVector32, Crypto, PimpSatoshi, PrivateKey, PublicKey, SatoshiLong, Script, ScriptFlags, Transaction}
...
trait TestVectorsSpec extends AnyFunSuite with Logging {
  ...
  def getFeerate(name: String): FeeratePerKw = FeeratePerKw(PimpSatoshi(tests(name)("local_feerate_per_kw").trim.toLong))
