...
import fr.acinq.bitcoin.{Block, ByteVector32, ByteVector64, PimpSatoshi, PrivateKey, PublicKey, SatoshiLong}
...
class LightningMessageCodecsSpec extends AnyFunSuite {
  ...
  def bin32(fill: Byte) = new ByteVector32(bin(32, fill).toArray)

  def scalar(fill: Byte) = new PrivateKey(ByteVector.fill(32)(fill).toArray)

  def point(fill: Byte) = new PrivateKey(ByteVector.fill(32)(fill).toArray).publicKey

  def publicKey(fill: Byte) = new PrivateKey(ByteVector.fill(32)(fill).toArray).publicKey
  ...
  test("encode/decode init message") {
    ...
    val chainHash1 = new ByteVector32("0101010101010101010101010101010101010101010101010101010101010101")
    val chainHash2 = new ByteVector32("0202020202020202020202020202020202020202020202020202020202020202")
    val testCases = Seq(
  ...
  test("encode/decode all channel messages") {
    ...
    val update_fee = UpdateFee(randomBytes32, FeeratePerKw(PimpSatoshi(2)))
    ...
    val unknown_message = UnknownMessage(tag = 60000, data = ByteVector32.One.toByteArray)
  ...
  test("Unknown messages") {
    ...
    val unknown = UnknownMessage(tag = 47282, data = ByteVector32.Zeroes.toByteArray)
  ...
  test("decode channel_update with htlc_maximum_msat") {
    ...
    assert(update === ChannelUpdate(ByteVector64.fromValidHex("58fff7d0e987e2cdd560e3bb5a046b4efe7b26c969c2f51da1dceec7bcb8ae1b634790503d5290c1a6c51d681cf8f4211d27ed33a257dcc1102862571bf17923"), ByteVector32.fromValidHex("06226e46111a0b59caaf126043eb5bbf28c34f3a5e332a1fc7b2b73cf188910f"), ShortChannelId(0x5a10000020000L), 1539791129, 1, 1, CltvExpiryDelta(6), 1 msat, 1 msat, 10, Some(980000000 msat)))
    val nodeId = PublicKey.fromHex("03370c9bac836e557eb4f017fe8f9cc047f44db39c1c4e410ff0f7be142b817ae4")
