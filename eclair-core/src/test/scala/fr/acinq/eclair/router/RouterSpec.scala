...
class RouterSpec extends BaseRouterSpec {
  ...
  test("properly announce valid new channels and ignore invalid ones") { fixture =>
...

    {
      ...
      watcher.send(router, ValidateResult(chan_ac, Right(new Transaction(0, Nil, new TxOut(1000000 sat, write(pay2wsh(Scripts.multiSig2of2(funding_a, funding_c)))) :: Nil, 0), UtxoStatus.Unspent)))
...

    {
      ...
      watcher.send(router, ValidateResult(chan_uc, Right(new Transaction(0, Nil, new TxOut(2000000 sat, write(pay2wsh(Scripts.multiSig2of2(priv_funding_u.publicKey, funding_c)))) :: Nil, 0), UtxoStatus.Unspent)))
