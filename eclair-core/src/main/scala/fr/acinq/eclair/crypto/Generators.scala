...
object Generators {
  ...
  def perCommitSecret(seed: Array[Byte], index: Long): PrivateKey = perCommitSecret(new ByteVector32(seed), index)
  ...
  def perCommitPoint(seed: Array[Byte], index: Long): PublicKey = perCommitPoint(new ByteVector32(seed), index)
