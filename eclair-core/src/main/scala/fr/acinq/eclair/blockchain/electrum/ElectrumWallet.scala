...
import fr.acinq.bitcoin.{Base58, Base58Check, Block, ByteVector32, DeterministicWallet, KeyPath, OP_PUSHDATA, OutPoint, Satoshi, Script, ScriptElt, ScriptWitness, SigVersion, Transaction, TxIn, TxOut}
import fr.acinq.bitcoin.{ByteVector => ByteVectorAcinq}
import fr.acinq.eclair.{KotlinUtils, LongToBtcAmount}
import fr.acinq.eclair.blockchain.bitcoind.rpc.Error
import fr.acinq.eclair.blockchain.electrum.ElectrumClient._
import fr.acinq.eclair.blockchain.electrum.db.{HeaderDb, WalletDb}
import fr.acinq.eclair.transactions.Transactions
import grizzled.slf4j.Logging
import scodec.bits.{ByteVector => ByteVectorScodec}
 ...
 * @param params
 */
class ElectrumWallet(seed: ByteVectorScodec, client: ActorRef, params: ElectrumWallet.WalletParameters) extends FSM[ElectrumWallet.State, ElectrumWallet.Data] {

  import Blockchain.RETARGETING_PERIOD
  import ElectrumWallet._
  import params._

  implicit def javain2scalain(input: java.util.List[TxIn]) : List[TxIn] = input.asScala.toList
  implicit def scalain2javain(input: Seq[TxIn]) : java.util.List[TxIn] = input.asJava
  implicit def javaout2scalaout(input: java.util.List[TxOut]) : List[TxOut] = input.asScala.toList
  implicit def scalaout2javaout(input: Seq[TxOut]) : java.util.List[TxOut] = input.asJava
  ...
  whenUnhandled {
    ...
    case Event(IsDoubleSpent(tx), data) =>
      ...
      val isDoubleSpent = data.heights
        ...
        .exists(spendingTx => spendingTx.txIn.asScala.map(_.outPoint).toSet.intersect(tx.txIn.asScala.map(_.outPoint).toSet).nonEmpty && spendingTx.txid != tx.txid) // look for a tx that spend the same utxos and has a different txid
...
object ElectrumWallet {
  def props(seed: ByteVectorScodec, client: ActorRef, params: WalletParameters): Props = Props(new ElectrumWallet(seed, client, params))
  ...
  case class SendAll(publicKeyScript: ByteVectorScodec, feeRatePerKw: Long) extends Request
   ...
   */
  def computeScriptHashFromPublicKey(key: PublicKey): ByteVector32 = new ByteVector32(Crypto.sha256(Script.write(computePublicKeyScript(key)))).reversed()

  def accountPath(chainHash: ByteVector32): KeyPath = chainHash match {
    // hardened(49) :: hardened(1) :: hardened(0) :: Nil
    case Block.RegtestGenesisBlock.hash | Block.TestnetGenesisBlock.hash => new KeyPath("m/49'/1'/0'") //
      // hardened(49) :: hardened(0) :: hardened(0) :: Nil
    case Block.LivenetGenesisBlock.hash =>  new KeyPath("m/49'/0'/0'")
  }
   ...
   */
  def accountKey(master: ExtendedPrivateKey, chainHash: ByteVector32) = {
    DeterministicWallet.derivePrivateKey(master, accountPath(chainHash).append(0))
  }
   ...
   */
  def changeKey(master: ExtendedPrivateKey, chainHash: ByteVector32) = DeterministicWallet.derivePrivateKey(master, accountPath(chainHash).append(1L))

  def totalAmount(utxos: Seq[Utxo]): Satoshi = new Satoshi(utxos.map(_.item.value).sum)
   ...
   */
  def computeFee(weight: Int, feeRatePerKw: Long): Satoshi = new Satoshi((weight * feeRatePerKw) / 1000)
  ...
  def extractPubKeySpentFrom(txIn: TxIn): Option[PublicKey] = {
    Try {
      ...
      val sig = txIn.witness.stack.get(0)
      val pub = txIn.witness.stack.get(1)
      val parsed = Script.parse(txIn.signatureScript)
      require(parsed.size() == 1)
      val script = parsed.get(0) match {
        case push: OP_PUSHDATA => push.data
      }
      //val OP_PUSHDATA(script, _) :: Nil = Script.parse(txIn.signatureScript)
      val publicKey = new PublicKey(pub)
      if (new ByteVectorAcinq(Script.write(Script.pay2wpkh(publicKey))) == script) {
        Some(publicKey)
      } else None
    } getOrElse None
  }
  ...
  case class Data(blockchain: Blockchain,
    ...
    lazy val publicScriptMap = (accountKeys ++ changeKeys).map(key => new ByteVectorAcinq(Script.write(computePublicKeyScript(key.publicKey))) -> key).toMap
    ...
    def isMine(txIn: TxIn): Boolean = extractPubKeySpentFrom(txIn).exists(pub => publicScriptMap.contains(new ByteVectorAcinq(Script.write(computePublicKeyScript(pub)))))
      ...
      history.get(scriptHash) match {
        ...
        case Some(items) =>
          ...
          val unspents = items collect { case item if transactions.contains(item.tx_hash) =>
            ...
            val outputs = tx.txOut.asScala.zipWithIndex.filter { case (txOut, index) => isReceive(txOut, scriptHash) }
          ...
          unspents.filterNot(utxo => txs.exists(tx => tx.txIn.asScala.exists(_.outPoint == utxo.outPoint)))
      }
    }
      ...
      history.get(scriptHash) match {
        ...
        case Some(items) =>
          ...
          def findOurSpentOutputs(txs: Seq[Transaction]): Seq[TxOut] = {
            val inputs = txs.flatMap(_.txIn.asScala).filter(txIn => isSpend(txIn, scriptHash))
          ...
          val confirmedReceived = confirmedTxs.flatMap(_.txOut.asScala).filter(txOut => isReceive(txOut, scriptHash))

          val unconfirmedSpents = findOurSpentOutputs(unconfirmedTxs)
          val unconfirmedReceived = unconfirmedTxs.flatMap(_.txOut.asScala).filter(txOut => isReceive(txOut, scriptHash))

          val confirmedBalance = confirmedReceived.map(_.amount).sum minus confirmedSpents.map(_.amount).sum
          val unconfirmedBalance = unconfirmedReceived.map(_.amount).sum minus unconfirmedSpents.map(_.amount).sum

          logger.debug(s"scriptHash=$scriptHash confirmedBalance=$confirmedBalance unconfirmedBalance=$unconfirmedBalance)")
          (new Satoshi(confirmedBalance), new Satoshi(unconfirmedBalance))
      }
    }
    ...
    lazy val balance: (Satoshi, Satoshi) = {
      ...
      balances.foldLeft((0 sat, 0 sat)) {
        case ((confirmed, unconfirmed), (confirmed1, unconfirmed1)) => (confirmed plus confirmed1, unconfirmed plus unconfirmed1)
      }
    }
    ...
    def computeTransactionDelta(tx: Transaction): Option[(Satoshi, Satoshi, Option[Satoshi])] = {
      val ourInputs = tx.txIn.asScala.filter(isMine)
      ...
      if (missingParent) {
        ...
        val sent = ourInputs.map(txIn => transactions(txIn.outPoint.txid).txOut.get(txIn.outPoint.index.toInt)).map(_.amount).sum
        val received = tx.txOut.asScala.filter(isMine).map(_.amount).sum
        // if all the inputs were ours, we can compute the fee, otherwise we can't
        val fee_opt = if (ourInputs.size == tx.txIn.size) Some(sent minus tx.txOut.asScala.map(_.amount).sum) else None
    ...
    def addUtxosWithDummySig(tx: Transaction, utxos: Seq[Utxo]): Transaction = {
      ...
      val inputs = utxos.map { case utxo =>
        val sig = new Array[Byte](71)
        val script = Script.write(Script.pay2wpkh(utxo.key.publicKey))
        val sigScript = new ByteVectorAcinq(Script.write((new OP_PUSHDATA(script) :: Nil).asJava))
        val witness = new ScriptWitness().push(sig).push(utxo.key.publicKey.value)
        new TxIn(utxo.outPoint, sigScript, TxIn.SEQUENCE_FINAL, witness)
      }
      tx.updateInputs(inputs)
...
//      })
    ...
    def completeTransaction(tx: Transaction, feeRatePerKw: Long, minimumFee: Satoshi, dustLimit: Satoshi, allowSpendUnconfirmed: Boolean): (Data, Transaction, Satoshi) = {
      ...
      def computeFee(candidates: Seq[Utxo], change: Option[TxOut]): Satoshi = {
        ...
        val fee = Transactions.weight2fee(feeRatePerKw, tx2.weight())
        fee
      }

      val dummyChange = new TxOut(new Satoshi(0), computePublicKeyScript(currentChangeKey.publicKey))

      @tailrec
      def loop(current: Seq[Utxo], remaining: Seq[Utxo]): (Seq[Utxo], Option[TxOut]) = {
        totalAmount(current) match {
          case total if (total minus computeFee(current, None)) < amount && remaining.isEmpty =>
            // not enough funds to send amount and pay fees even without a change output
            throw new IllegalArgumentException("insufficient funds")
          case total if (total minus computeFee(current, None)) < amount =>
            // not enough funds, try with an additional input
            loop(remaining.head +: current, remaining.tail)
          case total if (total minus computeFee(current, None)) <= (amount plus dustLimit) =>
            // change output would be below dust, we don't add one and just overpay fees
            (current, None)
          case total if (total minus computeFee(current, Some(dummyChange))) <= (amount plus dustLimit) && remaining.isEmpty =>
            // change output is above dust limit but cannot pay for it's own fee, and we have no more utxos => we overpay a bit
            (current, None)
          case total if (total minus computeFee(current, Some(dummyChange))) <= (amount plus dustLimit) =>
            // try with an additional input
            loop(remaining.head +: current, remaining.tail)
          case total =>
            val fee = computeFee(current, Some(dummyChange))
            val change = dummyChange.updateAmount(total minus amount minus fee)
            (current, Some(change))
        }
      }
      ...
      val fee = selected.map(s => new Satoshi(s.item.value)).sum minus tx3.txOut.asScala.map(_.amount).sum

      (data1, tx3, fee)
    }

    def signTransaction(tx: Transaction): Transaction = {
      val inputs = tx.txIn.asScala.zipWithIndex.map { case (txIn, i) =>
        ...
        val key = utxo.key
        val sig = Transaction.signInput(tx, i, Script.pay2pkh(key.publicKey), fr.acinq.bitcoin.SigHash.SIGHASH_ALL, new Satoshi(utxo.item.value), SigVersion.SIGVERSION_WITNESS_V0, key.privateKey)
        val sigScript = Script.write((new OP_PUSHDATA(Script.write(Script.pay2wpkh(key.publicKey))) :: Nil).asJava)
        val witness = new ScriptWitness((new fr.acinq.bitcoin.ByteVector(sig) :: key.publicKey.value :: Nil).asJava)
        txIn.updateSignatureScript(sigScript).updateWitness(witness)
      }
      tx.updateInputs(inputs.asJava)
...
//      })
    ...
    def commitTransaction(tx: Transaction): Data = {
      ...
      val history1 = (tx.txIn.asScala.filter(isMine).map(extractPubKeySpentFrom).flatten.map(computeScriptHashFromPublicKey) ++ tx.txOut.asScala.filter(isMine).map(_.publicKeyScript).map(computeScriptHash))
     ...
     */
    def spendAll(publicKeyScript: ByteVectorScodec, feeRatePerKw: Long): (Transaction, Satoshi) = {
      // use confirmed and unconfirmed balance
      val amount = balance._1 plus balance._2
      val tx = new Transaction(2, Nil, new TxOut(amount, publicKeyScript.toArray) :: Nil, 0)
      ...
      val tx2 = tx1.updateOutputs(new TxOut((amount minus fee), publicKeyScript.toArray) :: Nil)
      val tx3 = signTransaction(tx2)
      (tx3, fee)
    }

    def spendAll(publicKeyScript: Seq[ScriptElt], feeRatePerKw: Long): (Transaction, Satoshi) = spendAll(ByteVectorScodec.view(Script.write(publicKeyScript.asJava)), feeRatePerKw)
  }
