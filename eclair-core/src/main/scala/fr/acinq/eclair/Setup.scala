...
class Setup(datadir: File,
  ...
  def bootstrap: Future[Kit] = {
    for {
      ...
      postRestartCleaner = system.actorOf(PostRestartHtlcCleaner.props(nodeParams, register, Some(postRestartCleanUpInitialized)), "post-restart-htlc-cleaner")
      ...
      switchboard = system.actorOf(SimpleSupervisor.props(Switchboard.props(nodeParams, watcher, relayer, postRestartCleaner, wallet), "switchboard", SupervisorStrategy.Resume))
