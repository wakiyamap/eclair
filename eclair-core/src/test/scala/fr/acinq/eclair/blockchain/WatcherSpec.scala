...
import fr.acinq.bitcoin.{Base58, OutPoint, PrivateKey, Script, ScriptFlags, ScriptWitness, SigHash, SigVersion, Transaction, TxIn, TxOut}
...
class WatcherSpec extends AnyFunSuiteLike {
  ...
  test("extract pay2wpkh pubkey script") {
    ...
    assert(commitTx.txOut.head.publicKeyScript.contentEquals(WatchConfirmed.extractPublicKeyScript(claimMainTx.txIn.head.witness)))
  }
  ...
  test("extract pay2wsh pubkey script") {
    ...
    assert(commitTx.txOut.head.publicKeyScript.contentEquals(WatchConfirmed.extractPublicKeyScript(claimMainDelayedTx.txIn.head.witness)))
  }
...
object WatcherSpec {
  ...
  def getNewAddress(bitcoincli: ActorRef)(implicit system: ActorSystem): (String, PrivateKey) = {
    ...
    val (priv, true) = pair2tuple(PrivateKey.fromBase58(wif, Base58.Prefix.SecretKeyTestnet))
    (address, priv)
  }
  ...
  def createUnspentTxChain(tx: Transaction, priv: PrivateKey): (Transaction, Transaction) = {
    ...
    // tx1 spends tx
    val tx1 = {
      val tmp = new Transaction(2, new TxIn(new OutPoint(tx, outputIndex), Nil, TxIn.SEQUENCE_FINAL) :: Nil, new TxOut(tx.txOut(outputIndex).amount minus fee, Script.pay2wpkh(pub)) :: Nil, 0)
      ...
      val tmp1 = tmp.updateWitness(0, new ScriptWitness().push(sig).push(pub.value))
      Transaction.correctlySpends(tmp1, tx :: Nil, ScriptFlags.STANDARD_SCRIPT_VERIFY_FLAGS)
      tmp1
    }
    // and tx2 spends tx1
    val tx2 = {
      val tmp = new Transaction(2, new TxIn(new OutPoint(tx1, 0), Nil, TxIn.SEQUENCE_FINAL) :: Nil, new TxOut(tx1.txOut.head.amount minus fee, Script.pay2wpkh(pub)) :: Nil, 0)
