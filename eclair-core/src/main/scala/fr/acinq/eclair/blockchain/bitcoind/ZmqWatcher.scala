...
class ZmqWatcher(chainHash: ByteVector32, blockCount: AtomicLong, client: ExtendedBitcoinClient)(implicit ec: ExecutionContext = ExecutionContext.global) extends Actor with ActorLogging {
  ...
  def publishCommitWithAnchor(commitTx: Transaction, currentFeerate: FeeratePerKw, targetFeerate: FeeratePerKw, dustLimit: Satoshi, signingKit: TransactionSigningKit.ClaimAnchorOutputSigningKit): Future[ByteVector32] = {
    ...
    if (targetFeerate <= currentFeerate) {
      ...
      val dummyChangeAmount = Transactions.weight2fee(anchorFeerate, Transactions.claimAnchorOutputMinWeight) plus dustLimit
      publish(commitTx, isRetry = false).flatMap(commitTxId => {
        val txNotFunded = new Transaction(2, Nil, new TxOut(dummyChangeAmount, Script.pay2wpkh(Transactions.PlaceHolderPubKey)) :: Nil, 0)
        ...
        fundTxResponse.changePosition match {
          case Some(changePos) =>
            ...
            val txSingleOutput = fundTxResponse.tx.updateOutputs(Seq(changeOutput.updateAmount(changeOutput.amount plus dummyChangeAmount)))
          ...
          case None =>
            client.getChangeAddress()(singleThreadExecutionContext).map(pubkeyHash => {
              val txSingleOutput = fundTxResponse.tx.updateOutputs(Seq(new TxOut(dummyChangeAmount, Script.pay2wpkh(pubkeyHash.toArray))))
        ...
        val unsignedTx = txWithInput.copy(tx = fundTxResponse.tx.updateInputs(txWithInput.tx.txIn.head +: fundTxResponse.tx.txIn))
        adjustAnchorOutputChange(unsignedTx, commitTx, fundTxResponse.amountIn plus Transactions.AnchorOutputsCommitmentFormat.anchorAmount, currentFeerate, targetFeerate, dustLimit)
  ...
  def publishHtlcTx(currentFeerate: FeeratePerKw, targetFeerate: FeeratePerKw, dustLimit: Satoshi, signingKit: TransactionSigningKit.HtlcTxSigningKit): Future[ByteVector32] = {
    ...
    if (targetFeerate <= currentFeerate) {
      ...
      val txNotFunded = txWithInput.tx.updateInputs(Nil).updateOutputs(txWithInput.tx.txOut.head.updateAmount(dustLimit) :: Nil)
      ...
      client.fundTransaction(txNotFunded, FundTransactionOptions(targetFeerate * weightRatio, lockUtxos = true, changePosition = Some(1)))(singleThreadExecutionContext).map(fundTxResponse => {
        ...
        val txWithHtlcInput = fundTxResponse.tx
          .updateInputs(txWithInput.tx.txIn ++ fundTxResponse.tx.txIn)
          .updateOutputs(txWithInput.tx.txOut ++ fundTxResponse.tx.txOut.tail)
        ...
        adjustHtlcTxChange(unsignedTx, fundTxResponse.amountIn plus unsignedTx.input.txOut.amount, targetFeerate, dustLimit, signingKit)
      }).flatMap(unsignedTx => {
        ...
        client.signTransaction(signedHtlcTx.tx, Seq(inputInfo), allowIncomplete = true)(singleThreadExecutionContext).flatMap(signTxResponse => {
          ...
          val completeTx = signedHtlcTx.tx.updateInputs(signedHtlcTx.tx.txIn.head +: signTxResponse.tx.txIn.tail)
...
object ZmqWatcher {
  ...
  def adjustAnchorOutputChange(unsignedTx: Transactions.ClaimAnchorOutputTx, commitTx: Transaction, amountIn: Satoshi, currentFeerate: FeeratePerKw, targetFeerate: FeeratePerKw, dustLimit: Satoshi): Transactions.ClaimAnchorOutputTx = {
    ...
    val targetFee = Transactions.weight2fee(targetFeerate, estimatedWeight) minus Transactions.weight2fee(currentFeerate, commitTx.weight())
    val amountOut = dustLimit.max(amountIn minus targetFee)
    unsignedTx.copy(tx = unsignedTx.tx.updateOutputs(unsignedTx.tx.txOut.head.updateAmount(amountOut) :: Nil))
  }
  ...
  def adjustHtlcTxChange(unsignedTx: Transactions.HtlcTx, amountIn: Satoshi, targetFeerate: FeeratePerKw, dustLimit: Satoshi, signingKit: TransactionSigningKit.HtlcTxSigningKit): Transactions.HtlcTx = {
    ...
    val changeAmount = amountIn minus dummySignedTx.tx.txOut.head.amount minus targetFee
    if (dummySignedTx.tx.txOut.length == 2 && changeAmount >= dustLimit) {
      unsignedTx match {
        case htlcSuccess: HtlcSuccessTx => htlcSuccess.copy(tx = htlcSuccess.tx.updateOutputs(Seq(htlcSuccess.tx.txOut.head, htlcSuccess.tx.txOut(1).updateAmount(changeAmount))))
        case htlcTimeout: HtlcTimeoutTx => htlcTimeout.copy(tx = htlcTimeout.tx.updateOutputs(Seq(htlcTimeout.tx.txOut.head, htlcTimeout.tx.txOut(1).updateAmount(changeAmount))))
      }
    } else {
      unsignedTx match {
        case htlcSuccess: HtlcSuccessTx => htlcSuccess.copy(tx = htlcSuccess.tx.updateOutputs(Seq(htlcSuccess.tx.txOut.head)))
        case htlcTimeout: HtlcTimeoutTx => htlcTimeout.copy(tx = htlcTimeout.tx.updateOutputs(Seq(htlcTimeout.tx.txOut.head)))
      }
    }
  }
