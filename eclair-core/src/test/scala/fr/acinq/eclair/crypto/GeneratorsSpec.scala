...
class GeneratorsSpec extends AnyFunSuite {
  val base_secret: PrivateKey = new PrivateKey(Hex.decode("000102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f"))
  val per_commitment_secret: PrivateKey = new PrivateKey(Hex.decode("1f1e1d1c1b1a191817161514131211100f0e0d0c0b0a09080706050403020100"))
  val base_point = new PublicKey(Hex.decode("036d6caac248af96f6afa7f904f550253a0f3ef3f5aa2fe6838a95b216691468e2"))
  val per_commitment_point = new PublicKey(Hex.decode("025f7117a78150fe2ef97db7cfc83bd57b2e2c0d0dd25eaf467a4a1c2a45ce1486"))
  ...
  test("derivation of secret key from basepoint secret and per-commitment-secret") {
    ...
    assert(localprivkey.value == ByteVector32.fromValidHex("cbced912d3b21bf196a766651e436aff192362621ce317704ea2f75d87e7be0f"))
  }
  ...
  test("derivation of revocation secret from basepoint-secret and per-commitment-secret") {
    ...
    assert(revocationprivkey.value.toHex == "d09ffff62ddb2297ab000cc85bcb4283fdeb6aa052affbc9dddcf33b61078110")
  }
}
