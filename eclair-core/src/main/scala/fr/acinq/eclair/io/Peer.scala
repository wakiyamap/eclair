 ...
 */
class Peer(val nodeParams: NodeParams, remoteNodeId: PublicKey, watcher: ActorRef, relayer: ActorRef, paymentHandler: ActorRef, wallet: EclairWallet) extends FSMDiagnosticActorLogging[Peer.State, Peer.Data] {
  ...
  when(CONNECTED) {
    ...
    case Event(c: Peer.OpenChannel, d: ConnectedData) =>
      if (c.fundingSatoshis >= Channel.MAX_FUNDING && !nodeParams.features.hasFeature(Wumbo)) {
        ...
        if (canUseFeature(d.localInit.features, d.remoteInit.features, StaticRemoteKey)) channelVersion = channelVersion | ChannelVersion.STATIC_REMOTEKEY
        if (c.pushMsat > 0.msat) channelVersion = channelVersion | ChannelVersion.ZERO_RESERVE
        val (channel, localParams) = createNewChannel(nodeParams, funder = true, c.fundingSatoshis, origin_opt = Some(sender), channelVersion = channelVersion)
  ...
  private val msgLogger = new BusLogging(context.system.eventStream, "", classOf[Peer.MessageLogs]) with DiagnosticLoggingAdapter
...
object Peer {
  ...
  def props(nodeParams: NodeParams, remoteNodeId: PublicKey, watcher: ActorRef, relayer: ActorRef, paymentHandler: ActorRef, wallet: EclairWallet): Props = Props(new Peer(nodeParams, remoteNodeId, watcher, relayer, paymentHandler, wallet))
  ...
  case class ConnectedData(address: InetSocketAddress, peerConnection: ActorRef, localInit: wire.Init, remoteInit: wire.Init, channels: Map[ChannelId, ActorRef], payToOpenReqs: Map[ByteVector32, List[PayToOpen]] = Map.empty) extends Data
