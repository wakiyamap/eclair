...
object Transactions {
  ...
  object InputInfo {
    def apply(outPoint: OutPoint, txOut: TxOut, redeemScript: Seq[ScriptElt]) = new InputInfo(outPoint, txOut, new ByteVector(Script.write(redeemScript)))
    def apply(outPoint: OutPoint, txOut: TxOut, redeemScript: Array[Byte]) = new InputInfo(outPoint, txOut, new ByteVector(redeemScript))
  }
  ...
  def htlcOutputFee(feeratePerKw: Long, commitmentFormat: CommitmentFormat): MilliSatoshi = weight2feeMsat(feeratePerKw, commitmentFormat.htlcOutputWeight)
  ...
  def commitTxFeeMsat(dustLimit: Satoshi, spec: CommitmentSpec, commitmentFormat: CommitmentFormat): MilliSatoshi = {
    ...
    weight2feeMsat(spec.feeratePerKw, weight) + anchorsCost.toMilliSatoshi
  }
  ...
  def makeCommitTxOutputs(localIsFunder: Boolean,
    ...
    trimOfferedHtlcs(localDustLimit, spec, commitmentFormat).foreach { htlc =>
      val redeemScript = htlcOffered(localHtlcPubkey, remoteHtlcPubkey, localRevocationPubkey, htlc.add.paymentHash.ripemd160(), commitmentFormat)
    ...
    trimReceivedHtlcs(localDustLimit, spec, commitmentFormat).foreach { htlc =>
      val redeemScript = htlcReceived(localHtlcPubkey, remoteHtlcPubkey, localRevocationPubkey, htlc.add.paymentHash.ripemd160(), htlc.add.cltvExpiry, commitmentFormat)
  ...
  def makeClaimHtlcSuccessTx(commitTx: Transaction,
    ...
    val redeemScript = htlcOffered(remoteHtlcPubkey, localHtlcPubkey, remoteRevocationPubkey, htlc.paymentHash.ripemd160(), commitmentFormat)
  ...
  def makeClaimHtlcTimeoutTx(commitTx: Transaction,
    ...
    val redeemScript = htlcReceived(remoteHtlcPubkey, localHtlcPubkey, remoteRevocationPubkey, htlc.paymentHash.ripemd160(), htlc.cltvExpiry, commitmentFormat)
    outputs.zipWithIndex.collectFirst {
  ...
  def makeClaimP2WPKHOutputTx(commitTx: Transaction, localDustLimit: Satoshi, localPaymentPubkey: PublicKey, localFinalScriptPubKey: ByteVector, feeratePerKw: Long): Either[TxGenerationSkipped, ClaimP2WPKHOutputTx] = {
  ...
  def makeClaimRemoteDelayedOutputTx(commitTx: Transaction, localDustLimit: Satoshi, localPaymentPubkey: PublicKey, localFinalScriptPubKey: ByteVector, feeratePerKw: Long): Either[TxGenerationSkipped, ClaimRemoteDelayedOutputTx] = {
  ...
  def makeClaimDelayedOutputTx(commitTx: Transaction, localDustLimit: Satoshi, localRevocationPubkey: PublicKey, toLocalDelay: CltvExpiryDelta, localDelayedPaymentPubkey: PublicKey, localFinalScriptPubKey: ByteVector, feeratePerKw: Long): Either[TxGenerationSkipped, ClaimDelayedOutputTx] = {
  ...
  def makeHtlcPenaltyTx(commitTx: Transaction, htlcOutputIndex: Int, redeemScript: ByteVector, localDustLimit: Satoshi, localFinalScriptPubKey: Array[Byte], feeratePerKw: Long): Either[TxGenerationSkipped, HtlcPenaltyTx] =
  ...
  def addSigs(claimDelayedOutputTx: ClaimDelayedOutputTx, localSig: ByteVector64): ClaimDelayedOutputTx = {
  ...
  def checkSig(txinfo: TransactionWithInputInfo, sig: ByteVector64, pubKey: PublicKey, txOwner: TxOwner, commitmentFormat: CommitmentFormat): Boolean = {
    ...
    val data = Transaction.hashForSigning(txinfo.tx, 0, txinfo.input.redeemScript.toByteArray, sighash, txinfo.input.txOut.amount, SIGVERSION_WITNESS_V0)
