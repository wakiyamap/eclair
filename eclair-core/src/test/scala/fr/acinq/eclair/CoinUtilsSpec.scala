...
import fr.acinq.bitcoin.{Btc, MilliBtc, PimpSatoshi, Satoshi}
import org.scalatest.funsuite.AnyFunSuite
import org.scalatest.ParallelTestExecution

class CoinUtilsSpec  extends AnyFunSuite with ParallelTestExecution {
  ...
  test("Convert any BtcAmount to a raw BigDecimal in a given unit") {
    ...
    assert(CoinUtils.rawAmountInUnit(PimpSatoshi(123), BtcUnit) == BigDecimal(0.00000123))
    assert(CoinUtils.rawAmountInUnit(PimpSatoshi(123), MBtcUnit) == BigDecimal(0.00123))
    assert(CoinUtils.rawAmountInUnit(PimpSatoshi(123), BitUnit) == BigDecimal(1.23))
    assert(CoinUtils.rawAmountInUnit(PimpSatoshi(123), SatUnit) == BigDecimal(123))
    assert(CoinUtils.rawAmountInUnit(PimpSatoshi(123), MSatUnit) == BigDecimal(123000))
