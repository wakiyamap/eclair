...
case class PimpSatoshi(private val underlying: Long) extends BtcAmount with Ordered[PimpSatoshi] {
  // @formatter:off
  ...
  def max(other: BtcAmount): PimpSatoshi = other match {
    ...
    case other: MilliBtc => if (underlying > other.toSatoshi.toLong) this else other.toSatoshi
    case other: Btc => if (underlying > other.toSatoshi.toLong) this else other.toSatoshi
  }
  ...
  override def toSatoshi: Satoshi = new Satoshi(underlying)
  def toMilliSatoshi = MilliSatoshi(underlying * 1000)
  // @formatter:on
}
