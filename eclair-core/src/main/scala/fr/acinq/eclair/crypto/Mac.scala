...
object Hmac256 {
  def apply(key: Array[Byte]) : Hmac256 = new Hmac256(ByteVector.view(key))
}

object Mac32 {

  def hmac256(key: ByteVector, message: Array[Byte]): ByteVector32 = {
  ...
  def hmac256(key: ByteVector, message: ByteVector): ByteVector32 = hmac256(key, message.toArray)
}