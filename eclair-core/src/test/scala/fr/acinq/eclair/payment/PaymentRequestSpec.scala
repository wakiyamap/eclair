...
class PaymentRequestSpec extends AnyFunSuite {
  ...
  val priv = new PrivateKey(Hex.decode("e126f68f7eafcc8b74f54d269fe206be715000f94dac067d1c04a8ca3b2db734"))
  val pub = priv.publicKey
  val nodeId = pub
  assert(nodeId == new PublicKey(Hex.decode("03e7156ae33b0a208d0744199163177e909e80176e55d97a2f221ede0f934dd9ad")))
  ...
  test("Now send $24 for an entire list of things (hashed)") {
    ...
    assert(pr.timestamp == 1496314658L)
    assert(pr.nodeId == PublicKey.fromHex("03e7156ae33b0a208d0744199163177e909e80176e55d97a2f221ede0f934dd9ad"))
    assert(pr.description == Right(new ByteVector32(Crypto.sha256(ByteVector("One piece of chocolate cake, one icecream cone, one pickle, one slice of swiss cheese, one slice of salami, one lollypop, one piece of cherry pie, one sausage, one cupcake, and one slice of watermelon".getBytes)))))
    assert(pr.fallbackAddress === None)
    assert(pr.tags.size === 2)
    assert(PaymentRequest.write(pr.sign(priv)) == ref)
  }
  ...
  test("On mainnet, please send $30 for coffee beans to the same peer, which supports features 9, 15 and 99, using secret 0x1111111111111111111111111111111111111111111111111111111111111111") {
    ...
    for (ref <- refs) {
      ...
      assert(pr.paymentHash.toHex == "0001020304050607080900010203040506070809000102030405060708090102")
      assert(pr.paymentSecret === Some(new ByteVector32("1111111111111111111111111111111111111111111111111111111111111111")))
      assert(pr.timestamp === 1496314658L)
      assert(pr.nodeId === PublicKey.fromHex("03e7156ae33b0a208d0744199163177e909e80176e55d97a2f221ede0f934dd9ad"))
  ...
  test("On mainnet, please send 0.00967878534 BTC for a list of items within one week, amount in pico-BTC") {
    ...
    assert(pr.paymentHash.toHex == "462264ede7e14047e9b249da94fefc47f41f7d02ee9b091815a5506bc8abf75f")
    ...
    assert(pr.routingInfo === Seq(Seq(ExtraHop(PublicKey.fromHex("03d06758583bb5154774a6eb221b1276c9e82d65bbaceca806d90e20c108f4b1c7"), ShortChannelId("589390x3312x1"), 1000 msat, 2500, CltvExpiryDelta(40)))))
    assert(pr.features.supported)
    assert(PaymentRequest.write(pr.sign(priv)) === ref)
  }
  ...
  test("correctly serialize/deserialize variable-length tagged fields") {
    ...
    val pr = PaymentRequest(chainHash = Block.LivenetGenesisBlock.hash, amount = Some(123 msat), paymentHash = new ByteVector32("01" * 32), privateKey = priv, description = "Some invoice", expirySeconds = Some(123456), timestamp = 12345)
  ...
  test("ignore unknown tags") {
    val pr = PaymentRequest(
      ...
      nodeId = nodeId,
      tags = List(
        PaymentHash(new ByteVector32("01" * 32)),
