...
class ElectrumWalletSimulatedClientSpec extends TestKitBaseClass with AnyFunSuiteLike {
  ...
  val wallet = TestFSMRef(new ElectrumWallet(ByteVector.view(seed), client.ref, walletParameters))
  ...
  test("disconnect if server sends an invalid transaction") {
    ...
    val TransactionReceived(_, _, amount1, _, _, _) = listener.expectMsgType[TransactionReceived]
    assert(amount1 == new Satoshi(100000))
    // we think we have some unconfirmed funds
    val WalletReady(amount2, _, _, _) = listener.expectMsgType[WalletReady]
    assert(amount2 == new Satoshi(100000))
    ...
    probe.send(wallet, GetMerkleResponse(tx.txid, ByteVector32.fromValidHex("01" * 32) :: Nil, 2, 0, None))
    watcher.expectTerminated(probe.ref)
    awaitCond(wallet.stateName == ElectrumWallet.DISCONNECTED)
  ...
  test("handle pending transaction requests") {
    ...
    val scriptHash = computeScriptHashFromPublicKey(key.publicKey)
    wallet ! ScriptHashSubscriptionResponse(scriptHash, "02" * 32)
    client.expectMsg(GetScriptHashHistory(scriptHash))
...
object ElectrumWalletSimulatedClientSpec {
  def makeHeader(previousHeader: BlockHeader, timestamp: Long): BlockHeader = {
    var template = previousHeader.setHashPreviousBlock(previousHeader.hash).setTime(timestamp).setTime(0)
